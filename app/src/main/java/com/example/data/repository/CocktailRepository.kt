...
class CocktailRepository(private val dao: CocktailDao) {
    ...
    suspend fun exportCustomBackupJson(): String {
        ...
        return com.example.util.BackupJsonManager.exportToJson(
     ...
     */
    suspend fun importBackupJson(jsonString: String): com.example.util.ImportResult {
        return try {
            val (ingredients, recipes) = com.example.util.BackupJsonManager.parseBackupJson(jsonString)
            ...
            com.example.util.ImportResult(
                importedRecipesCount = importedRecipeCount,
                importedIngredientsCount = importedIngCount
            )
        } catch (e: Exception) {
            com.example.util.ImportResult(0, 0, e.message ?: "Failed to parse backup JSON file.")
        }
    }
